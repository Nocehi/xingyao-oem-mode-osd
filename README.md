...
Quickshell OSD for the calibrated Fn+X OEM performance/balanced modes on the
MECHREVO Xingyao 14 P916F-STX under Linux.
...
MESSAGE=input <the same inputN>: Unknown key pressed, code: 0x0041|0x0042
```

The two calibrated scancodes become stable semantic values and are sent to a
standalone Quickshell OSD through argv-only IPC. The listener does not grab or
inject input, use an evdev fallback, call `journalctl`, change power limits, or
persist a guessed current mode. A repeated report of the same mode inside 250
ms is debounced; a distinct calibrated mode is never discarded as a duplicate.
...
All `zh*` locales display `性能模式`, `均衡模式`, and the manual compatibility
fallback `OEM 模式已切換`; other locales display `Performance mode`, `Balanced
mode`, and `OEM mode switched`. IPC tokens remain English.
...
commands are not part of the deterministic hardware-free gate. Neither is
physical Fn+X, power-limit, IPC, or visual proof.
...
For a direct source-tree OSD smoke without Fn+X:
...
The production listener uses the zero-argument `showPerformance` and
`showBalanced` wrappers observed with noctalia-qs 0.0.12, whose `ipc call`
command rejects positional function arguments. An accepted IPC response is not
proof that a compositor visibly presented the surface.

For a disposable session-dependent round-trip of all three zero-argument
wrappers, including the manual unknown-mode fallback, run:

```sh
./tests/live-qml-smoke.sh
```

It briefly displays the OSD and terminates only the temporary Quickshell
process it creates. It is not part of headless CI, and its success still needs a
human or captured visual observation before making a presentation claim.
...
The release gate does not simulate Fn+X, the SMU, RyzenAdj readback, live
journal rotation, IPC timeout behavior, session-environment propagation,
multi-output placement, or physical OSD presentation. The current listener also
waits synchronously for each `qs ipc` child and has no child timeout.
